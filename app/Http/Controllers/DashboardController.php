...
class DashboardController extends Controller
{
    ...
    public function prosesResetPassword(Request $request){
        ...
        if(Hash::check($old_password, $user->password)){
            ...
            try{
                $user->save();
                return redirect(route('dashboard.resetPassword'))->with('pesan',['success','Selamat anda Berhasil Mengubah Password']);
            }catch (\Exception $e){
                return redirect(route('dashboard.resetPassword'))->with('pesan',['danger','Anda Gagal Mengubah Password']);
            }
        }else{
